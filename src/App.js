...
import { AppBar, Toolbar, Divider, Drawer, Box, Typography, CssBaseline, FormGroup, FormControlLabel, Switch, Slider } from '@mui/material';
...
function loadMetaData(projections, n){
  const metadata_sample = require(`/public/projections/${projections}/0/metadata.json`);
  ...
  ({
    value: Array.from({length: n}, (v, i) => {
      let metadata = require(`/public/projections/${projections}/${i}/metadata.json`);
...
function App(props) {
  ...
  useEffect(() => {
    ...
    sortValueRef.current.all = require(`/public/projections/${projections}/${sortMetric}.json`);
  ...
  useEffect(() => {
    ...
    metadataRef.current = loadMetaData(projections, n);
  ...
  useEffect(() => {
    showValueRef.current.all = require(`/public/projections/${projections}/${showMetric}.json`);
    // 보여질값 새로 가져옴
